...
// WiFi Credentials
const char* ssid = "KOST BERSAMA";
const char* password = "NGENTANK";
...
void handleRoot() {
  ...
  String html = "<!DOCTYPE html>"
                ...
                "<p id='clock'>Clock: " + getTimeString(hour(), minute()) + ":" + String(second()) + "</p>"
  ...
  html += "</div><br>"
          ...
          "  document.getElementById('clock').innerHTML = 'Clock: ' + jam + ':' + menit + ':' + detik;"
          "}"
...
void loop() {
  ...
  if (now - lastNTP >= 60000) { // Update NTP setiap 60 detik
    lastNTP = now;
    if (WiFi.status() == WL_CONNECTED) {
      timeClient.update();
      setTime(timeClient.getEpochTime()); // Sinkronkan TimeLib
    }
  }
